...
**아쉬운 점**
- 중반까지 구조화된 설계를 유지했으나 마무리 단계에서 개발 속도가 급격히 저하됨
- 일정 관리 실패로 물고기 성장 후 스프라이트 변경 등 콘텐츠 추가 실패

**배운 점**
- SO + 상속 계층이 데이터 확장성에서 Prefab 대비 압도적으로 관리가 쉬움
- 코루틴 사용 시 `WaitForSeconds` · `WaitUntil` 캐싱으로 GC 스파이크 방지

**다음에는**
- 초기 설계 단계에서 매니저 책임 범위를 명확히 분리 (SRP 준수)
- 최적화를 고려한 에셋 임포트 설정부터 챙기기
